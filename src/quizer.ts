...
(() => {
    ...
    function finishSession(timeout: boolean) {
        ...
        (document.getElementById("finish-description") as HTMLElement).innerHTML = timeout
    ...
    function processResponse(response: WorkerResponse) {
        ...
        if (taskDescription && response.pass && lastRequest && lastRequest.code.length <= taskDescription.target) {
    ...
    function runTest() {
        ...
        if (!editor || isTestRunning || !taskDescription) {
